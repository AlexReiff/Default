...
public class SegmentedLeastSquares {
	...
	public void computeOptIterative() {	  
		...
		//determine best fit for all lengths
		for(int j = 2; j < opt.length; j++) {
			minCost = Double.MAX_VALUE;
			for(int i = 1; i <= j; i++) {
				double currCost = opt[i-1] + costSegment + e_ij[i][j];
				if(currCost < minCost) {
					minCost = currCost;
				}
			}
	...
	public double computeOptRecursive(int j){
		...
		//memoization: it won't recompute a value
		if(opt[j] != 0) {
			return opt[j];
		}
		...
		//determine best fit
		for(int i = 1; i <= j; i++) {
			double currCost = computeOptRecursive(i-1) + e_ij[i][j] + costSegment;
			if(currCost < minCost) {
				minCost = currCost;
			}
		}
	...
	public void computeSegmentation(int j) {
		...
		//for all points
		if (j > 0) {
			//find the line segment that ends on that point
			for(int i = 0; i <= j; i++) {
				if(i == 0) {
					currVal = e_ij[i][j] + costSegment;
				}
				else {
					currVal = opt[i-1] + e_ij[i][j] + costSegment;
				}
				//if it is the correct line segment, add it
				if (opt[j] == currVal) {
					temp = new LineSegment(i,j,a[i][j],b[i][j], e_ij[i][j]);
					lineSegments.add(temp);
					computeSegmentation(i-1);
					//otherwise it will finish each for loop after finsing the segments
					break;
				}
			}
		}
