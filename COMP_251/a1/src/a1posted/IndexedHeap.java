...
public class IndexedHeap{   
	...
	private void swap(int i, int j){
		//----------------------- ADD YOUR CODE HERE ----------------------------		
		String iNode = names.get(i);
		double iVal = priorities.get(i);
		String jNode = names.get(j);
		double jVal = priorities.get(j);
		
		nameToIndex.put(iNode, j);
		nameToIndex.put(jNode, i);
		priorities.set(j, iVal);
		priorities.set(i, jVal);
		names.set(j, iNode);
		names.set(i, jNode);
	}
	...
	public String removeMin(){
		//----------------------- ADD YOUR CODE HERE  ----------------------------
		String minNode = nameOfMin();
		String maxNode = names.get(names.size() - 1);
		double maxPriority = priorities.get(priorities.size() - 1);
		
		//move the lowest priority node to the root
		names.set(1, maxNode);
		priorities.set(1, maxPriority);
		names.remove(names.size() - 1);
		priorities.remove(priorities.size() - 1);
		nameToIndex.remove(minNode);
