...
*)
  ...
  let rec merge s1 s2 = {
    hd = if s1.hd > s2.hd then s2.hd else s1.hd;
    tl = if s1.hd = s2.hd then 
         Susp (fun() -> merge (force s1.tl) (force s2.tl))
         else if s1.hd > s2.hd then Susp (fun() -> merge s1 (force s2.tl))
         else Susp (fun() -> merge (force s1.tl) s2)
  }
