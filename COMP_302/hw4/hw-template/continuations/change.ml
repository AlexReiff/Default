...
let rec change coins amt sc fc = let backCoins = List.rev coins in
  if amt = 0 then sc []
  else match backCoins with
  | [] -> fc ()
  | (coin::t) -> if coin > amt then change (List.rev t) amt sc fc
      else change (coins) (amt - coin) (fun r -> sc (coin::r)) fc
