...
let rec unusedVars e = match e with 
  ...
  | Bool b -> []
  | If(e, e1, e2) ->
    union (unusedVars e, union (unusedVars e1, unusedVars e2))
  | Primop (po, args) ->
    List.fold_right (fun e1 e2 -> union (unusedVars e1, e2)) args []
  | Tuple exps ->
    unionList (List.map unusedVars exps)
  | Fn (x, _, e) -> if List.mem x (freeVars e) then [] else [x]
  | Rec (x, _, e) -> if List.mem x (freeVars e) then [] else [x]
  | Let (decs, e2) ->
      let (free, bound) = varsDecs decs in
      union (free, delete (bound, unusedVars e2))
  | Apply (e1, e2) ->
      union(unusedVars e1, unusedVars e2)
  | Anno (e, _) -> unusedVars e
...
and eval exp =
  ...
  | Let(d, e2) -> (match d with 
    | [] -> eval e2
    | Val (exp, name)::t -> subst ((eval exp), name) (Let(t,e2))
    | Valtuple (exp, namelist)::t -> raise Unimplemented
    | ByName (exp, name)::t -> subst (exp,name) (Let(t,e2)))
  ...
  | Apply (e1, e2) -> (match eval e1, eval e2 with
    | Fn (x,t,e), v2 -> subst (v2, x) (Fn(x,t,e))
    | _, _ -> raise (Stuck "Left term of application is not an Fn")) 
